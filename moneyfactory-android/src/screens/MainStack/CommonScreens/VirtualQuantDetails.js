...
const VirtualQuantDetails = ({route, orders}) => {
  ...
  const handleDeployment = async () => {
    if (subscribe) {
      ...
      ToastAndroid.show(
        `Please subscribe the quant first.`,
        ToastAndroid.SHORT,
      );
    ...
    <SafeAreaView style={[tw('h-full w-full'), styles.container]}>
      <View style={[tw('h-full '), styles.container]}>
        <LinearGradient
          ...
          <MyStatusBar padding={15} transparent />
          <View style={[tw('my-3')]}>
            <Header
            ...
            <ScrollView
              ...
              {selectedList?.map((listItem, index) => {
                return (
                  <TouchableOpacity
                    key={index}
                    style={[tw('flex items-center flex-row  mt-3')]}
                    onPress={() => {
                      setDeploy(listItem);
...
export default VirtualQuantDetails;
